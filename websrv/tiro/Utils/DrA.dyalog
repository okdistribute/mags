 ...
 ⍝ --- Error Handling ---
   ...
   ⍝ Gather information for error logging
      ...
      :If 0≠⍴⍙99.t←(2=|⌊⎕NC ⍙99.t)/⍙99.t ⍝ Any are variables?
          ⍙99.t←⍙99.t,(⍴∘⍎¨⍙99.t),(≡∘⍎¨⍙99.t),(⎕SIZE ⍙99.t),[1.5]⊂⍬  ⍝ Names, shape, depth, sizes, slots for data
     
          :If 0≠⍴⍙99.n←(⍙99.t[;4]<100000)/⍳⊃⍴⍙99.t ⍝ Log values of vars with size < 100,000 bytes
   ...
   ⍝ Insight Systems - original function by Kim Andresen
   ...
   ⍝ Make the DrA form. _Q: 1=User Mode, 2=Developer Mode
      ...
      '#'⎕WS'3D' 'Plinth'
      'MAIN'⎕WC'Form'('Dr. Insight says ...',(0≠⍴_F)/' [',_F,']')('Size' 600 800)('Coord' 'Pixel')('Font' 'Dyalog Std')('OnTop' 1)('Border' 2)('Event' 'Close' 1),_E
      ...
      :If _Q=2 ⍝ Developer
          ...
          :If 'D'=4 1⊃'.'⎕WG'APLVersion'
              'MAIN.Debug'⎕WC'Button' 'Debug'_P,_B,⊂('Tip' 'Exit from Dr. Insight to the APL Session')
          :EndIf
   ...
   ⍝ 'MAIN.End'⎕WC'Button' 'Exit'_P,_B,⊂('Tip' 'Terminate ',AppName)')
    ...
    ∇ F←Info MakeFile Dir;⎕TRAP;get;Z;D;F;TN;TS;MKDIR;old
   ⍝ Create a DrA Error Log file under the installation directory
     ...
     ELSE:
      ⎕←⎕DM
      (old NoUser)←NoUser 0 ⍝ Stop even if NoUser>0
...
⍝ --- Mail Tools ---
   ...
   ⍝ Send
    ...
    ∇ z←{att}SendSMTP text
   ⍝ Send message to dra@insight.dk via SMTP
