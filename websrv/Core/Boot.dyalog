:Namespace Boot
    ...
    ∇ Run root;Config
      ...
      Config.Port←Config Setting'Port' 1
      Config.TrapErrors←Config Setting'TrapErrors' 1
      Config.Logger←Config Setting'Logger' 0 ''
      Config.SessionHandler←Config Setting'SessionHandler' 0 ''
      Config.Authentication←Config Setting'Authentication' 0 ''
      Config.Debug←Config Setting'Debug' 1
      Config.Root←MSRoot{((IsRelPath ⍵)/⍺),⍵}AppRoot
      Config.TempFolder←''{((IsRelPath ⍵)/⍺),⍵}Config Setting'TempFolder' 0
      ...
      :If 0≠⎕NC'#.DrA' ⍝ Transfer DrA config options
          {}#.DrA.SetDefaults
          #.DrA.Mode←2 ⍝ Developer mode
          #.DrA.Path←AppRoot ⍝ Where to put log files
          #.DrA.SMTP_Gateway←Config.SMTP_Gateway
          #.DrA.MailRecipient←Config.MailRecipient
          #.DrA.MailMethod←Config.MailMethod
...
⍝      ConfigureLogger ms
    ...
    ∇ ConfigureDatasources ms;file;ds;name;tmp;orig
      ...
      :AndIf 0<⍴ms.Datasources←,(#.XML.ToNS #.Files.GetText file).Datasources
          ...
          :For ds :In ms.Datasources
              :For name :In ds.⎕NL ¯2
                  orig←tmp←ds.⍎name
                  tmp←(#.Strings.subst∘('%ServerRoot%'(¯1↓MSRoot)))tmp
                  tmp←(#.Strings.subst∘('%SiteRoot%'(¯1↓AppRoot)))tmp
                  :If orig≢tmp
                      ⍎'ds.',name,'←tmp'
                  :EndIf
              :EndFor
          ...
          :Trap 0
              ...
              :If ⍱/0 ¯1≠1⊃#.SQA.Init'' ⍝ and initialize
    ...
    ∇ ConfigureResources ms;file;mask;inds;names;uses;map;n;res
      ⍝ load virtual resource definitions if any
      ...
      :AndIf 0<⍴res←,(#.XML.ToNS #.Files.GetText file).Resources
          ...
          ms.Config.Resources←names,(SubstPath¨{⍵~⊂''}∘∪¨map/¨⊂res Setting'script'),[1.1]SubstPath¨{⍵~⊂''}∘∪¨map/¨⊂res Setting'style'
      :EndIf
    ∇
