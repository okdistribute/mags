...
⍝ Fields related to the Response
    ...
    ∇ Make(cmd data);buf;input;args;req;hdrs;i;z;pars
...
⍝ Arguments: Arguments to the command (cmd?arg1=value1&arg2=value2) as 2 column matrix of name/value pairs
      ...
      Response←⎕NS''
      Response.(Status StatusText Headers File HTML HTMLHead PeerAddr NoWrap)←200 'OK'(0 2⍴⊂'')0 '' '' '' 0
      Response.(TS MSec)←⎕TS(⎕AI[3])
...
⍝ PeerCert←conns.PeerCert
      ...
      :If (1↑⍴hdrs)≥i←hdrs[;1]⍳⊂'cookie'
          Cookies←↑{((#.Strings.lc(1⊃⍵)~' '))(DeCode 2⊃⍵)}¨';='Split⊃hdrs[i;2]
      :Else
          Cookies←0 2⍴⊂''
      :EndIf
    ...
    ⍝ Translate HTTP command line arguments
      ...
      :AndIf 0≠⍴i←(i<¯1+⍴r)/i
          z←r[j←i∘.+1 2]
          r[i]←⎕UCS 16⊥⍉¯1+('0123456789ABCDEF'⍳z)⌊'0123456789abcdef'⍳z
          m←(⍴r)⍴1 ⋄ m[j]←0
          r←m/r
      :EndIf
    ∇
 ...
 ⍝ Decode HTML Header
    ...
    ∇ Fail x;i;f;root
      ...
      :EndIf
      :If #.Files.Exists f←root,'ErrorPages\',(⍕x),'.htm'
          Response.HTML←(#.Files.GetText f),'<br/>',Response.HTML
      :EndIf
    ∇
    ...
    ∇ Use x;n;ind
      ...
      :AndIf ~0∊n←1↑⍴Server.Config.Resources
          :If n≥ind←Server.Config.Resources[;1]⍳⊂x
              (Script∘'')¨{'src="',⍵,'"'}¨(⊂ind 2)⊃Server.Config.Resources
              Style¨(⊂ind 3)⊃Server.Config.Resources
          :Else
