...
⍝original          success←'success: function(d){if(typeof(d.replace)!="undefined"){$(d.replace).html(d.data);}else if(typeof(d.append)!="undefined"){$(d.append).append(d.data);}else if(typeof(d.prepend)!="undefined"){$(d.prepend).prepend(d.data);}else if(typeof(d.execute)!="undefined"){eval(d.execute);}}'
      :Else
          dtype←'"html"'
          success←'success: function(d){$(',(quote resp),').empty().html(d);}'
      :EndIf
     
      ajax←'$.ajax({url: ',page,', cache: false, type: "POST", dataType: ',dtype,', data: {',data,'}, ',success,'});'
