...
⍝ Modal Message
    ∇ r←{req}PopupMessage pars;ih;title;jqpars;modal
      req←{6::'' ⋄ req}⍬
      pars←eis pars
      (ih title modal jqpars)←4↑pars,(⍴pars)↓'' '' 1 ''
      ih←CRLF~⍨#.HTMLInput.quote'div'#.HTMLInput.Enclose ih
      jqpars,←(0∊⍴jqpars)↓(title ine',title:',(#.HTMLInput.quote title)),(modal/',modal:true'),',onClose:function(){$(this).dialog("destroy");}'
      r←req #.JQUI.Dialog ih'' ''jqpars
    ∇
...
⍝ DataTables
    ...
    ⍝ the first row is considered a header row
      ...
      :Else
          :If 2=⍴⍴tablepars ⋄ tablepars←,⊂tablepars ⋄ :EndIf
          tablepars←tablepars,(⍴,tablepars)↓(0 0⍴⊂'')tab'' '' 1 ⍝ see HTMLInput.Table for
          r←id #.HTMLInput.Table tablepars
      :EndIf
