...
(define free-vars
  (lambda (expr env)
    (define help
      (lambda (expr env acc)
	(cond
	 [(or (number? expr) (boolean? expr)) acc]
	 [(symbol? expr) (if (assoc expr env) acc (cons expr acc))]
	 [else  (help (car expr) env
		      (help (cadr expr) env
			    (help (caddr expr) env acc)))])))
    (help expr env '())))
...
(define tree-sort
  (lambda (rel? ls)
    (inorder (list->bst rel? ls))))

;; 8a
(define sort-by-weight
  (lambda (wlist)
    (tree-sort (lambda (x y)
		 (< (cadr x) (cadr y)))
	       wlist)))
